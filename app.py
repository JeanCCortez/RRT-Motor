...
# ==========================================
# DICIONÁRIO ABSOLUTO (SEM ATALHOS, 100% MAPEADO)
# ==========================================
LANG = {
    "PT": {
        ...
        "rep_red_text": "LAUDO PREDITIVO:\n1. DESAFIO: A massa de lente informada ({mest_obs}x10^11) gera uma curvatura clássica irrisória. O Modelo Padrão exigiria halos massivos para forçar a solução.\n2. CONVERGÊNCIA TRR: Varrendo o tecido cósmico com base na refração da constante Beta, o algoritmo prediz matematicamente que, para formar o anel de {tobs} arcsec, a galáxia fonte deve estar posicionada em z_S = {zs_pred:.4f}.\nRESULTADO: Precisão de calibração cruzada de {prec:.2f}%.",
        "rep_str_text": "LAUDO DE HIDRODINÂMICA:\n1. A astrofísica clássica afirma que os 'gaps' da corrente estelar são colisões com pedaços invisíveis de matéria.\n2. A TRR rastreou a órbita inteira medindo as forças de maré. O Cisalhamento Viscoso atingiu o limite crítico de ruptura na zona exata de {loc_str}. O gap é um atrito com o fluido do espaço."
    },
    "EN": {
        ...
        "info_red": "💡 TRR iterates the gravitational matrix to predict the source galaxy's distance (z_S).",
        "info_str": "💡 TRR maps vacuum tension and reveals the fake sub-halo coordinates.",
        "pred_zs": "Predicted Redshift z_S", "loc_gap": "📌 Fake Sub-halo Coordinates", "no_gap": "No critical rupture",
        "pdf_h1": "REFERENTIAL RELATIVITY THEORY (TRR)", "pdf_h2": "Automated Audit Report", "pdf_footer": "Document generated by RRT Cosmological Engine.",
        ...
        "rep_dyn_text": "TECHNICAL REPORT:\n1. Baryonic mass yields only {vbar:.2f} km/s, far from the real {vobs:.2f} km/s.\n2. TRR calculates topological friction proportional to orbital circumference. Applying Beta (0.028006), fluid drag elevates velocity to {vtrr:.2f} km/s.\nRESULT: {prec:.2f}% accuracy achieved without Dark Matter.",
        "rep_opt_text": "TECHNICAL REPORT:\nVisible mass deflects light by only {tbar:.2f} arcsec. TRR applies Time Refraction (eta_C = {etac:.5f}). Phase delay widens the ring to {ttrr:.2f} arcsec, matching the telescope. Accuracy: {prec:.2f}%.",
        "rep_red_text": "PREDICTIVE REPORT:\n1. CHALLENGE: Lens mass ({mest_obs}x10^11) generates minimal classical curvature. The Standard Model would require massive halos.\n2. TRR CONVERGENCE: Sweeping cosmic fabric based on Beta refraction, the algorithm mathematically predicts the source galaxy is at z_S = {zs_pred:.4f} to form the {tobs} arcsec ring.\nRESULT: Cross-calibration accuracy of {prec:.2f}%.",
        "rep_str_text": "HYDRODYNAMICS REPORT:\n1. Classical astrophysics claims stream 'gaps' are invisible collisions.\n2. TRR tracked the orbit measuring tidal forces. Viscous Shear hit critical rupture limits exactly at {loc_str}. The gap is vacuum fluid friction."
    },
    "ES": {
        ...
        "info_red": "💡 TRR itera la matriz gravitacional para predecir la distancia de la galaxia fuente (z_S).",
        "info_str": "💡 TRR mapea la tensión del vacío y revela las coordenadas del falso sub-halo.",
        ...
        "rep_dyn_text": "DICTAMEN TÉCNICO:\n1. La masa bariónica genera solo {vbar:.2f} km/s.\n2. TRR calcula la fricción topológica. Aplicando Beta (0.028006), el arrastre eleva a {vtrr:.2f} km/s.\nRESULTADO: Precisión de {prec:.2f}% sin Materia Oscura.",
        "rep_opt_text": "DICTAMEN TÉCNICO:\nMasa visible desvía luz en {tbar:.2f} arcsec. TRR aplica Refracción Temporal (eta_C = {etac:.5f}). El retraso de fase amplía a {ttrr:.2f} arcsec. Precisión: {prec:.2f}%.",
        "rep_red_text": "DICTAMEN PREDICTIVO:\nUsando la masa exacta, TRR aísla la refracción del vacío y predice matemáticamente que la galaxia fuente está en z_S = {zs_pred:.4f}. Precisión: {prec:.2f}%.",
        "rep_str_text": "MECÁNICA FLUIDA:\nLa astrofísica clásica inventa colisiones invisibles. TRR rastreó la órbita y detectó Cizallamiento Viscoso crítico en la zona: {loc_str}. El gap es fricción del vacío."
    },
    "FR": {
        ...
        "info_red": "💡 TRR prédit la distance temporelle (z_S) en utilisant strictement la Masse Photométrique.",
        "info_str": "💡 TRR cartographie le Cisaillement Visqueux et délivre les coordonnées de rupture.",
        ...
        "rep_dyn_text": "RAPPORT TECHNIQUE:\n1. La masse baryonique génère {vbar:.2f} km/s.\n2. TRR (Beta) élève la vitesse à {vtrr:.2f} km/s. Précision: {prec:.2f}%.",
        "rep_opt_text": "RAPPORT TECHNIQUE:\nTRR applique Réfraction Temporelle (eta_C = {etac:.5f}). Déviation amplifiée à {ttrr:.2f} arcsec. Précision: {prec:.2f}%.",
        "rep_red_text": "PRÉDICTION:\nEn utilisant la masse exacte, TRR prédit le Redshift Source à z_S = {zs_pred:.4f}. Précision: {prec:.2f}%.",
        "rep_str_text": "MÉCANIQUE FLUIDE:\nTRR a détecté un Cisaillement Visqueux critique dans la zone: {loc_str}. Les halos noirs sont obsolètes."
    },
    "DE": {
        ...
        "rep_dyn_text": "GUTACHTEN:\n1. Masse erzeugt nur {vbar:.2f} km/s.\n2. TRR-Widerstand erhöht auf {vtrr:.2f} km/s. Genauigkeit: {prec:.2f}%.",
        "rep_opt_text": "GUTACHTEN:\nTRR wendet Zeitbrechung an (eta_C = {etac:.5f}). Ablenkung auf {ttrr:.2f} arcsec verstärkt. Genauigkeit: {prec:.2f}%.",
        "rep_red_text": "VORHERSAGE:\nOhne ad-hoc-Anpassungen prognostiziert TRR das Quellen-Redshift auf z_S = {zs_pred:.4f}. Genauigkeit: {prec:.2f}%.",
        "rep_str_text": "FLUIDMECHANIK:\nTRR erkannte kritische viskose Scherung in der Zone: {loc_str}. Gaps sind Vakuumspannung."
    },
    "IT": {
        ...
        "info_red": "💡 TRR prevede la distanza temporale (z_S) usando strettamente la Massa Fotometrica.",
        "info_str": "💡 TRR mappa il Taglio Viscoso e fornisce le coordinate esatte.",
        ...
        "rep_dyn_text": "DIAGNOSI:\nLa massa genera solo {vbar:.2f} km/s. TRR eleva a {vtrr:.2f} km/s. Precisione: {prec:.2f}%.",
        "rep_opt_text": "DIAGNOSI:\nRifrazione Temporale (eta_C = {etac:.5f}). Deviazione amplificata a {ttrr:.2f} arcsec. Precisione: {prec:.2f}%.",
        "rep_red_text": "PREVISIONE:\nUsando la massa esatta, TRR prevede il Redshift Sorgente in z_S = {zs_pred:.4f}. Precisione: {prec:.2f}%.",
        "rep_str_text": "MECCANICA FLUIDA:\nTRR ha rilevato Taglio Viscoso critico nella zona: {loc_str}. Aloni oscuri obsoleti."
    },
    "ZH": {
        ...
        "pdf_h1": "REFERENTIAL RELATIVITY THEORY (TRR)", "pdf_h2": "Automated Audit Report", "pdf_footer": "Document generated by RTT Cosmological Engine.",
        "pdf_title_dyn": "SCIENTIFIC AUDIT - DYNAMICS", "pdf_title_opt": "SCIENTIFIC AUDIT - OPTICS", "pdf_title_red": "SCIENTIFIC AUDIT - REDSHIFT", "pdf_title_str": "SCIENTIFIC AUDIT - STREAMS",
        "rep_dyn_text": "诊断:\n重子质量产生 {vbar:.2f} km/s. TRR 阻力提高到 {vtrr:.2f} km/s. 精度: {prec:.2f}%.",
        "rep_opt_text": "诊断:\nTRR (eta_C = {etac:.5f}) 放大偏转至 {ttrr:.2f} arcsec. 精度: {prec:.2f}%.",
        "rep_red_text": "预测:\nTRR 隔离真空折射，预测光源红移 (z_S) 为 {zs_pred:.4f}. 精度: {prec:.2f}%.",
        "rep_str_text": "预测流体力学:\nTRR 在区域 {loc_str} 检测到关键粘性剪切. 缝隙纯粹是真空张力."
    },
    "RU": {
        ...
        "info_red": "💡 TRR прогнозирует временное расстояние (z_S), строго используя предоставленную массу.",
        ...
        "pdf_h1": "REFERENTIAL RELATIVITY THEORY (TRR)", "pdf_h2": "Automated Audit Report", "pdf_footer": "Document generated by RRT Cosmological Engine.",
        "pdf_title_dyn": "SCIENTIFIC AUDIT - DYNAMICS", "pdf_title_opt": "SCIENTIFIC AUDIT - OPTICS", "pdf_title_red": "SCIENTIFIC AUDIT - REDSHIFT", "pdf_title_str": "SCIENTIFIC AUDIT - STREAMS",
        "rep_dyn_text": "ДИАГНОЗ:\nБарионная масса дает {vbar:.2f} км/с. Сопротивление TRR увеличивает скорость до {vtrr:.2f} км/с. Точность: {prec:.2f}%.",
        "rep_opt_text": "ДИАГНОЗ:\nTRR (eta_C = {etac:.5f}) усиливает отклонение до {ttrr:.2f} arcsec. Точность: {prec:.2f}%.",
        "rep_red_text": "ПРОГНОЗ:\nTRR прогнозирует Redshift источника z_S = {zs_pred:.4f}. Точность: {prec:.2f}%.",
        "rep_str_text": "ГИДРОДИНАМИКА:\nTRR обнаружила критический сдвиг вакуума в зоне: {loc_str}. Разрывы - это вакуумное натяжение."
    }
}
...
def criar_grafico_redshift(z_vals, theta_class, theta_trr, zs_pred, theta_obs):
    ...
    ax.plot(z_vals, theta_trr, color='#3498db', linewidth=2, label="TRR Predictive Curve")
    ...
    ax.set_title("TRR Cosmological Target Convergence", fontsize=11)
    ax.grid(alpha=0.3)
...
# ==========================================
# INTERFACE DO STREAMLIT (4 ABAS BLINDADAS)
# ==========================================
...
if st.session_state['idioma_selecionado'] is None:
    ...
    with aba1:
        ...
        if 'res_dyn' in st.session_state:
            ...
            st.download_button(L["pdf_btn"], data=gerar_pdf("dyn", res, L), file_name="Report_Din.pdf", mime="application/pdf", use_container_width=True, key="p1")
    ...
    with aba2:
        ...
        if 'res_opt' in st.session_state:
            ...
            st.download_button(L["pdf_btn"], data=gerar_pdf("opt", res, L), file_name="Report_Opt.pdf", mime="application/pdf", use_container_width=True, key="p2")
